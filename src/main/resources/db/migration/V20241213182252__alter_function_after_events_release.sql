...
BEGIN
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        ...
        IF new.account_id IS NOT NULL AND new.credit_card_id IS NOT NULL THEN
            RAISE EXCEPTION 'The release must have an account or credit card ID, not both';
        END IF;
    END IF;
    ...
    ELSEIF TG_OP = 'UPDATE' THEN
        ...
        ELSEIF ( -- só faz algo se houver alteração no valor, na conta, no cartão ou na conta de destino da transferência
            ...
            IF (
                (coalesce(new.account_id, -1) = coalesce(old.account_id, -1))
                    AND
                (coalesce(new.target_account_id, -1) = coalesce(old.target_account_id, -1))
                    AND
                (coalesce(new.credit_card_id, -1) = coalesce(old.credit_card_id, -1))
                    AND
            ...
            ELSE
                ...
                -- do nothing
                ELSEIF coalesce(new.account_id, -1) <> coalesce(old.account_id, -1) THEN -- atualizado de uma conta para outra conta
                    IF new.type = 'R' THEN
                        UPDATE account
                        SET balance = balance - old.amount
                        WHERE id = old.account_id;
